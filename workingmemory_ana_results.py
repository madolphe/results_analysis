...
def draw_all_distributions(dist_ind,dist_summary,num_dist,num_cond,std_val = 0.05,fname_save='workingmemory_accuracy.png'):
    ...
    axes.set_xticklabels(['4','5','6','7','8'],fontsize=20)
...
if __name__=='__main__':
    ...
    #data loading
    csv_path = "results/workingmemory.csv"
...
#condition extraction
    ...
    for ob in indices_id:
        ...
        sum_observers.append([np.sum(tmp_df.sum4),np.sum(tmp_df.sum5),np.sum(tmp_df.sum6),np.sum(tmp_df.sum7),np.sum(tmp_df.sum8)])
    ...
    draw_all_distributions(dist_ind,dist_summary,len(sum_observers),num_cond=5,std_val = 0.05,fname_save='workingmemory_accuracy.png')
