...
if __name__ == '__main__':
    ...
    for ob in indices_id:
        ...
        sum_observers.append([np.sum(tmp_df.result_correct),np.sum(tmp_df.result_nb_omission),np.mean(tmp_df.mean_result_clean_rt)])
    ...
    draw_all_distributions(dist_ind,dist_summary,len(sum_observers),num_cond=2,std_val = 0.05,fname_save='../outputs/gonogo/gonogo_hrfar.png')
    

    dist_ind = sum_observers.iloc[0:len(sum_observers),2].values
    dist_summary = extract_mu_ci_from_summary_rt(class_stan_rt)
    draw_all_distributions_rt(dist_ind,dist_summary,len(sum_observers),num_cond=1,std_val = 0.05,fname_save='../outputs/gonogo/gonogo_rt.png')
    print('finished')