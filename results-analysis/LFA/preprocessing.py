...
tasks_names = ['gonogo', 'moteval', 'taskswitch', 'loadblindness', 'memorability', 'enumeration']
import os

print(os.getcwd())
df_dict = {}
for task_name in tasks_names:
    df_dict[task_name] = pd.read_csv(f"../../outputs/{task_name}/sumdata_{task_name}.csv")
...
for col in df_dict['taskswitch'].columns:
    if col != 'participant_id':
        if 'rt' not in col and 'accuracy' not in col:
...
for i, (task_name, df) in enumerate(df_dict.items()):
    ...
    if i == 0:
        ...
        dataframe = pd.merge(dataframe, df, on='participant_id', how='outer')
    dataframe.to_csv('tmp_merge_acp.csv')
