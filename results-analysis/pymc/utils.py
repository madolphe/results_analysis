    ...
    for group in ['zpdes', 'baseline']:
        ...
        for task, (data, condition_list) in tasks.items():
            for condition in condition_list:
                summary_path = f"fixed_posterior/{task}_{group}_results/summary-{task}_{group}-{condition}.csv"
                try:
                    new_row = pd.read_csv(summary_path)
                except:
                    print("Path incorrect")
                pre_test = new_row[new_row['Unnamed: 0'] == "pre_test_posterior"]
                post_test = new_row[new_row['Unnamed: 0'] == "pre_test_posterior"]
                ...
                tmp_row = {'task_name': task,
                           ...
                           'post_97_HDI': pre_test['hdi_97%'].values[0],
